...
type ExportColumnKey = 'id' | 'firstname' | 'lastname' | 'email' | 'username';
...
export class ExportComponent implements OnChanges {
  ...
  private readonly defaultExportColumns: ExportColumnKey[] = ['id', 'firstname', 'lastname', 'email', 'username'];
  ...
  private buildExportUrl(format: 'pdf' | 'xlsx'): string {
    ...
    return `${this.backendBaseUrl}/api/reports/invoice/${format}?${params.toString()}`;
  }
  ...
  downloadPdf(): void {
    ...
    const fallback = `facepay-auth-log-${new Date().toISOString().slice(0, 10)}.pdf`;
    this.http
      .get(this.buildExportUrl('pdf'), {
      ...
      .subscribe({
        next: (res) => {
          const body = res.body;
          if (!body) {
            alert('Khong nhan duoc file PDF.');
            return;
          }
        ...
        error: (err) => {
          ...
          const msg = err?.status === 403 ? 'Ban khong co quyen admin.' : (err?.message ?? 'Loi tai PDF');
          alert(`Tai PDF loi: ${msg}`);
        },
        complete: () => {
          this.downloading = null;
  ...
  downloadXlsx(): void {
    ...
    this.downloading = 'xlsx';
    const fallback = `facepay-auth-log-${new Date().toISOString().slice(0, 10)}.xlsx`;
    this.http
      .get(this.buildExportUrl('xlsx'), {
        headers: this.authHeaders(),
        responseType: 'blob',
        observe: 'response'
      })
      .subscribe({
        next: (res) => {
          const body = res.body;
          if (!body) {
            alert('Khong nhan duoc file Excel.');
            return;
          }
          const name = this.filenameFromContentDisposition(res.headers.get('Content-Disposition'), fallback);
          this.saveBlob(body, name);
        ...
        error: (err) => {
          this.downloading = null;
          const msg = err?.status === 403 ? 'Ban khong co quyen admin.' : (err?.message ?? 'Loi tai Excel');
          alert(`Tai Excel loi: ${msg}`);
        },
        complete: () => {
          this.downloading = null;
          this.resetExportColumnsToDefault();
